...
The bounded context pattern in Domain-Driven Design defines fragments inside a domain model by decomposing a domain into bounded subdomains. In service-based environments the bounded context marks the boundaries of an application service. An application service is a concretion of the bounded context pattern. This is similar to domain modules in Angular in which we mark the boundries based on features. Applying the bounded context pattern to domain modules allows us to structure modules in a domain-driven approach. A bounded context should be presented at a minimum scale as aggregate. The bounded context must provide an REST API to enable navigatorial behaviour because the Angular router complies with the navigatorial nature of hypermedia APIs. A bounded context is only coupled to the URI of the entry point resource, then its Hypermedia to navigate through the API e.g. `/boundedContextA/*API`; `/boundedContextB/*API`. 
